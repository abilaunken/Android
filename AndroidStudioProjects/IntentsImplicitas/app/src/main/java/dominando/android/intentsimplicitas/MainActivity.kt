...
class MainActivity : AppCompatActivity() {
    ...
    private fun callNumber() {
        ...
        //  intent = Intent(Intent.ACTION_DIAL, uri) //Disca , nao precisa permissao
        intent = Intent(Intent.ACTION_CALL, uri) //Liga, precisa permissao
        openIntent(intent)
    }
    ...
    private fun openIntentAtPosition(position: Int) {
        ...
        when (position) {
            ...
            4 -> { //Compartilhar
                intent =
                    Intent().setAction(Intent.ACTION_SEND).putExtra(Intent.EXTRA_TEXT, "Compartilhamento via Intent.")
                        .setType("text/plain")
                openIntent(intent)
            }
            5 -> { //Alarme
                intent = Intent(AlarmClock.ACTION_SET_ALARM)
                    ...
                    .putExtra(
                        ...
                        arrayListOf(Calendar.MONDAY, Calendar.WEDNESDAY, Calendar.FRIDAY)
                    )
                openIntent(intent)
            }
            6 -> { //Busca na web
